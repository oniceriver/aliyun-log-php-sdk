...
*/
	...
	public static function skip_field($fp, $wire_type) {
		switch ($wire_type) {
			case 0: // varint
				return Protobuffer::skip_varint($fp);

			case 1: // 64bit
				if (fseek($fp, 8, SEEK_CUR) === -1)
					throw new \Exception('skip(' . static::get_wiretype(1) . '): Error seeking');
				return 8;

			case 2: // length delimited
				$varlen = 0;
				$len = static::read_varint($fp, $varlen);
				if (fseek($fp, $len, SEEK_CUR) === -1)
					throw new \Exception('skip(' . static::get_wiretype(2) . '): Error seeking');
				return $len - $varlen;
			...
			case 5: // 32bit
				if (fseek($fp, 4, SEEK_CUR) === -1)
					throw new \Exception('skip('. static::get_wiretype(5) . '): Error seeking');
				return 4;

			default:
				throw new \Exception('skip('. static::get_wiretype($wire_type) . '): Unsupported wire_type');
		}
	}
	...
	public static function read_field($fp, $wire_type, &$limit = null) {
		switch ($wire_type) {
			case 0: // varint
				return static::read_varint($fp, $limit);
			...
			case 2: // length delimited
				$len = static::read_varint($fp, $limit);
				$limit -= $len;
				return fread($fp, $len);
			...
			default:
				throw new \Exception('read_unknown('. static::get_wiretype($wire_type) . '): Unsupported wire_type');
		}
	}
